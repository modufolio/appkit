...
class UploadTest extends TestCase
{
    ...
    {
        $file = $this->createUploadedFile();
        $handler = Upload::from($file);

        $this->assertInstanceOf(Upload::class, $handler);
        $this->assertFalse($handler->hasErrors());
    }
    ...
    /**
     * Apache's mod_mime maps every extension segment, so `shell.php.jpg` is
     * served as PHP. Each segment is checked, not only the last.
     */
    public function testSaveToRejectsAnExecutableExtensionHiddenBeforeTheLast(): void
    {
        $tmpDir = sys_get_temp_dir().'/upload_test_'.uniqid();
        $handler = Upload::from($this->createUploadedFile('x', 'shell.php.jpg'));
        ...
        try {
            ...
            $this->assertFileDoesNotExist($tmpDir.'/shell.php.jpg');
            if (is_dir($tmpDir)) {
                rmdir($tmpDir);
            }
        }
    ...
    public function testSaveToRejectsInlineScriptableMarkup(): void
    {
        $tmpDir = sys_get_temp_dir().'/upload_test_'.uniqid();
        $handler = Upload::from($this->createUploadedFile('<script>', 'page.html'));
    ...
    public function testSaveToAcceptsAHarmlessMultiExtensionName(): void
    {
        $tmpDir = sys_get_temp_dir().'/upload_test_'.uniqid();
        $handler = Upload::from($this->createUploadedFile('data', 'archive.tar.gz'));

        try {
            $handler->saveTo($tmpDir);
            $this->assertFileExists($tmpDir.'/archive.tar.gz');
        } finally {
            if (file_exists($tmpDir.'/archive.tar.gz')) {
                unlink($tmpDir.'/archive.tar.gz');
            }
            if (is_dir($tmpDir)) {
                rmdir($tmpDir);
            }
