...
- **`Query\Segment` falls back to accessors.** A segment that names neither
  ...
  `movie.released_on` reaches `getReleasedOn()` on an object that has no
  `released_on` property or `releasedOn()`/`released_on()` method of its
  own. Only ever a fallback: an object that names the method or property
  itself is never second-guessed. Alongside, a
